...
# de 30° de tobogán de fibra de vidrio sin reiniciar SketchUp: edita
# este archivo y en la Consola de Ruby -Window > Ruby Console- corre
#   load '/Users/minorusal/Documents/SKETCHUP/scripts/codo_30_playidea.rb'
...
# tobogan_recto_playidea.rb, pero curvado 30°, con la misma ceja de
...
# arco de 30°, cada uno rotado según su ángulo, unidos consecutivos.

Object.send(:remove_const, :PlayIdeaCodo30Script) if defined?(PlayIdeaCodo30Script)

module PlayIdeaCodo30Script
  extend self

  DICTIONARY = 'playidea_tobogan'.freeze
  SEGMENTS = 48       # resolución de la circunferencia del tubo
  ANGLE_STEPS = 24    # resolución del barrido de 0 a 30°
  ...
  # Radio de curvatura del CENTRO del ducto -no del tubo-. PROVISIONAL:
  # igual al diámetro interno (800mm), pendiente de confirmar contra
  # una pieza real -ajusta este único número si no coincide-.
  BEND_RADIUS_MM = INSIDE_DIAMETER_MM

  COLOR = Sketchup::Color.new(225, 225, 232)
  CODE = 'TOBOGAN-CODO-30'.freeze

  def start
    Sketchup.active_model.select_tool(PlacementTool.new({}))
  rescue StandardError => error
    UI.messagebox("No fue posible iniciar el creador de codo de 30°:\n#{error.message}")
    puts error.full_message
  end

  # Un punto sobre el anillo del ducto a un ángulo `theta` del barrido
  # (0=entrada lisa, 30°=salida hacia la ceja) y `phi` alrededor de la
  ...
  # Punto/dirección de viaje en la SALIDA del barrido (theta=30°), para
  ...
  def build_piece(_params, start_point, direction, up_hint = Z_AXIS)
    ...
    model.start_operation('Crear codo de 30° de tobogán', true)

    definition = model.definitions.add(unique_name(model, CODE))
    entities = definition.entities

    # --- Tramo curvo: de theta=0 (liso) a theta=30° (hacia la ceja) ---
    ...
    definition.set_attribute(DICTIONARY, 'type', 'tobogan_codo_30')
  ...
  # hacia +X_AXIS y curva 30° en el plano X-Y -queda "acostado"-; gira la
  ...
  class PlacementTool
    ...
    def onLButtonDown(_flags, x, y, view)
      @input_point.pick(view, x, y)
      PlayIdeaCodo30Script.build_piece(@params, @input_point.position, X_AXIS)
