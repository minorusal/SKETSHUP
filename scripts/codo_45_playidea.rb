...
# de 45° de tobogán de fibra de vidrio sin reiniciar SketchUp: edita
# este archivo y en la Consola de Ruby -Window > Ruby Console- corre
#   load '/Users/minorusal/Documents/SKETCHUP/scripts/codo_45_playidea.rb'
...
# tobogan_recto_playidea.rb, pero curvado 45°, con la misma ceja de
...
# arco de 45°, cada uno rotado según su ángulo, unidos consecutivos.

Object.send(:remove_const, :PlayIdeaCodo45Script) if defined?(PlayIdeaCodo45Script)

module PlayIdeaCodo45Script
  extend self

  DICTIONARY = 'playidea_tobogan'.freeze
  SEGMENTS = 48       # resolución de la circunferencia del tubo
  ANGLE_STEPS = 24    # resolución del barrido de 0 a 45°
  ...
  CODE = 'TOBOGAN-CODO-45'.freeze

  def start
    Sketchup.active_model.select_tool(PlacementTool.new({}))
  rescue StandardError => error
    UI.messagebox("No fue posible iniciar el creador de codo de 45°:\n#{error.message}")
    puts error.full_message
  end

  # Un punto sobre el anillo del ducto a un ángulo `theta` del barrido
  # (0=entrada lisa, 45°=salida hacia la ceja) y `phi` alrededor de la
  ...
  # Punto/dirección de viaje en la SALIDA del barrido (theta=45°), para
  ...
  def build_piece(_params, start_point, direction, up_hint = Z_AXIS)
    ...
    model.start_operation('Crear codo de 45° de tobogán', true)

    definition = model.definitions.add(unique_name(model, CODE))
    entities = definition.entities

    # --- Tramo curvo: de theta=0 (liso) a theta=45° (hacia la ceja) ---
    ...
    definition.set_attribute(DICTIONARY, 'type', 'tobogan_codo_45')
  ...
  # hacia +X_AXIS y curva 45° en el plano X-Y -queda "acostado"-; gira la
  ...
  class PlacementTool
    ...
    def onLButtonDown(_flags, x, y, view)
      @input_point.pick(view, x, y)
      PlayIdeaCodo45Script.build_piece(@params, @input_point.position, X_AXIS)
