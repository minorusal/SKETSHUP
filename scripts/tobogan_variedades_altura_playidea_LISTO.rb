...
Object.send(:remove_const, :PlayIdeaToboganVariedadesAlturaScriptListo) if defined?(PlayIdeaToboganVariedadesAlturaScriptListo)

module PlayIdeaToboganVariedadesAlturaScriptListo
  extend self
  ...
  def elbow_count_and_pitch_for_height(target_height_mm, bend_radius_mm, ceja_mm, duct_outside_r_mm)
    ...
    while steady <= MAX_STEADY_ELBOW_COUNT
      ...
      if pitch_deg && pitch_deg <= MAX_REASONABLE_PITCH_DEG
        return [elbow_count, pitch_deg]
      end
  ...
  class PlacementTool
    ...
    def onLButtonDown(_flags, x, y, view)
      @input_point.pick(view, x, y)
      PlayIdeaToboganVariedadesAlturaScriptListo.build_all_variants(@input_point.position, X_AXIS)
...
PlayIdeaToboganVariedadesAlturaScriptListo.start
