...
Object.send(:remove_const, :PlayIdeaToboganCompletoScript) if defined?(PlayIdeaToboganCompletoScript)

module PlayIdeaToboganCompletoScript
  extend self
  ...
  def build_assembly(start_point, heading = X_AXIS)
    ...
    # --- Tercer brazo, hacia el CENTRO del 2º codo contando de ARRIBA
    ...
    # arm3_loop_i=1 porque duct_segments[1+2*i] es el codo, y el
    # conteo desde ARRIBA es i+1 -i=0 es el primer codo despues del
    # tramo recto, el mas alto de todos-.
    arm3_loop_i = 1
    ...
    arm3_target_point, = point_at_segment_fraction(arm3_elbow_segment, 0.5)
    arm3_drop_mm = 300.0 # mismo valor inicial que tenia el 2do brazo, ajustable
    ...
    # OJO: el radio de giro aqui es MUY chico -unos 280mm-, así que
    # tomar prestado el 850mm del 2do brazo da una vuelta de ~173°, y
    # la busqueda del ducto mas cercano -solo mira X,Y, ignora la
    # altura- termina enganchando OTRO codo distinto -reportado por el
    # usuario-. Regresado a 0 -sin girar- para que la solera busque en
    # el codo correcto; el brazo puede seguir atravesando el ducto por
    # ahora, pendiente de que el usuario diga cuanto y hacia donde
    # girarlo -sin copiar el valor del 2do brazo, aqui no aplica igual-.
    arm3_shift_mm = 0.0
    ...
    # Ancla para la pata/solera -ver arm2_leg_anchor_*-. Por ahora
    # SIGUE el giro en vivo del brazo -arm3_shift_mm-, para que la
    # solera quede pegada a donde realmente esta el conector/pata -no
    # separada, flotando en la posicion original sin girar-. El dia que
    # se quiera "congelar" en un punto especifico -como se hizo con el
    # brazo 2-, aqui se pone ese valor fijo en vez de arm3_shift_mm.
    arm3_leg_anchor_shift_mm = arm3_shift_mm
    arm3_leg_anchor_rotation_rad = -(arm3_leg_anchor_shift_mm / arm3_rotation_radius_mm)
    arm3_leg_anchor_rotation = Geom::Transformation.rotation(ORIGIN, Z_AXIS, arm3_leg_anchor_rotation_rad)
    arm3_leg_anchor_direction = arm3_direction_before_rotation.transform(arm3_leg_anchor_rotation).normalize
    arm3_leg_anchor_point = post_axis_at_arm3.offset(arm3_leg_anchor_direction, arm3_rotation_radius_mm.mm)
    ...
    arm3_duct_near_x_mm = arm3_leg_anchor_point.x.to_mm
    arm3_duct_near_y_mm = arm3_leg_anchor_point.y.to_mm
    arm3_duct_point, arm3_duct_tangent = nearest_duct_point(duct_segments, arm3_duct_near_x_mm, arm3_duct_near_y_mm)
    arm3_mid_leg_top_point = Geom::Point3d.new(arm3_leg_anchor_point.x, arm3_leg_anchor_point.y, arm3_duct_point.z)
    ...
    # Poste partido en 4 tramos -entre CON-61/CON-10 #1, CON-10 #1/
    # CON-10 #2, CON-10 #2/CON-10 #3, y CON-10 #3/arriba-, mas los 3
    # brazos horizontales.
    ...
    post_mid_length_mm = (arm2_z - arm_z).to_mm
  ...
  class PlacementTool
    ...
    def onLButtonDown(_flags, x, y, view)
      ...
      PlayIdeaToboganCompletoScript.build_assembly(@input_point.position, X_AXIS)
...
PlayIdeaToboganCompletoScript.start
