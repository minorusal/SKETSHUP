# Aro-collar de entrada del tramo recto, empacado dentro de
# constructor_modulos_playidea. Ver scripts/aro_entrada_tobogan_playidea.rb
# para el detalle de cómo se construyó -idéntica aquí-. Namespaced bajo
# PlayIdea::ConstructorModulos::Tobogan::AroEntrada.
  ...
  module ConstructorModulos
    module Tobogan
      module AroEntrada
        ...
        RECTO_BODY_OUTSIDE_R_MM = (800.0 / 2.0) + 8.0
        ...
        def build_piece(params, start_point, direction)
          ...
          z0 = 0.mm
          z1 = COLLAR_LENGTH_MM.mm
          z2 = z1 + FLANGE_THICKNESS_MM.mm

          inner_r = COLLAR_INNER_R_MM.mm
          outer_r = COLLAR_OUTER_R_MM.mm
          flange_r = FLANGE_OUTER_R_MM.mm

          add_wall(entities, inner_r, z0, z2)
          add_wall(entities, outer_r, z0, z1)
          add_wall(entities, flange_r, z1, z2)
          add_ring(entities, inner_r, outer_r, z0)
          add_ring(entities, outer_r, flange_r, z1)
          add_ring(entities, inner_r, flange_r, z2)

          soften_circular_edges(entities)

          material = aro_material(model, params[:color_hex])
        ...
        def aro_material(model, color_hex)
          hex = color_hex || DEFAULT_COLOR_HEX
          name = "PlayIdea Aro Entrada ##{hex}"
